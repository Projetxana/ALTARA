...
export const SanctuumProvider = ({ children }) => {
    ...
    const createBooking = async (bookingData) => {
        // Local generation of booking and cleaning tasks
        ...
        // Auto-generate cleaning task locally
        const newTask = {
            id: `t-${Date.now()}`,
            chaletId: bookingData.chaletId,
            bookingId: bookingId,
        ...
        // If user is connected, persist to Supabase
        if (user) {
            try {
                // Insert booking
                const { error: bookingError } = await supabase
                    ...
                    .insert([{
                        ...
                        status: 'confirmed',
                        color: '#d4af37',
                        total_revenue: bookingData.totalPrice || 0
                    }]);
                ...
                if (!bookingError) {
                    ...
                    console.error('[SanctuumContext] Error creating indirect direct booking:', bookingError);
                }
            } catch (err) {
                console.error('[SanctuumContext] Try/Catch error creating booking:', err);
            }
        }
