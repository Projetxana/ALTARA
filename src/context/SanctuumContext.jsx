...
import BookingService, { mapBookingFromDb } from '../features/booking/BookingService';
import CalendarBlockService, {
    mapCalendarBlockFromDb
} from '../features/booking/CalendarBlockService';
...
export const SanctuumProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const mappedBookings = (cloudBookings || [])
                .map(mapBookingFromDb)
                .filter(Boolean);
            ...
            const {
                data: cloudCalendarBlocks,
                error: calendarBlocksError
            } = await supabase
            ...
            if (calendarBlocksError) {
                console.error(
                    'Error fetching calendar blocks:',
                    calendarBlocksError
                );
            }

            const mappedCalendarBlocks =
                (cloudCalendarBlocks || [])
                    .map(mapCalendarBlockFromDb)
                    .filter(Boolean);

            setCalendarBlocks(mappedCalendarBlocks);
    ...
    const createBooking = async (bookingData) => {
        const createdBooking = await BookingService.createAdminBooking(
            bookingData
        );
        ...
        // Mirror the auto-generated cleaning task locally.
        setCleaningTasks(prev => [
            ...prev,
            {
                id: `local-${createdBooking.id}`,
                chaletId: createdBooking.chaletId,
                bookingId: createdBooking.id,
                date: createdBooking.checkOutDate,
                status: 'pending',
                autoGenerated: true
            }
        ]);
    ...
    const createCalendarBlock = async (blockData) => {
        const createdBlock =
            await CalendarBlockService.createBlock(blockData);
    ...
    const updateCalendarBlock = async (
        blockId,
        changes
    ) => {
        const updated =
            await CalendarBlockService.updateBlock(
                blockId,
                changes
            );
    ...
    const deleteCalendarBlock = async (
        ...
        await CalendarBlockService.deleteBlock(
            blockId
        );
